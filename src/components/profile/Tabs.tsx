...
const tabs = [
    { name: 'Mi Cuenta', href: '/profile', icon: UserIcon },                            // titulo, link y su icono
    { name: 'Cambiar Contraseña', href: '/profile/password', icon: FingerPrintIcon },
]

function classNames(...classes: string[]) {         // funcion para unir clases de tailwind en un string
    return classes.filter(Boolean).join(' ')
}

export default function Tabs() {
    const navigate = useNavigate()
    const location = useLocation()              // identifica ruta actual
        ...
        <div className='mb-10'>
            ...
            <div className="hidden sm:block">
                <div className="border-b border-gray-200">
                    <nav className="-mb-px flex space-x-8" aria-label="Tabs">       {/** tablet */}
                        {tabs.map((tab) => (            // itera sobre las rutas y le agrega su enlace, icono y titulo
                            <Link
                                key={tab.name}
                                to={tab.href}
                                className={classNames(                      // funcion  para agregar estilos al enlace              
                                    location.pathname === tab.href          // si la ruta coincide con la actual, le asigna un color diferente
